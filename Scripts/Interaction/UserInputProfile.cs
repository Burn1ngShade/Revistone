    ...
    namespace Interaction
    {
        ...
        public class UserInputProfile //really happy with this script atm
        {
            ...
            public bool InputValid(string input)
            {
                ...
                if (validInputTypes.Length != 0 && !validInputTypes.Contains(inputType)) errors.Add($"Input Recognised As [{inputType}], Should Be {ArrayToElementString(validInputTypes)}!");
                if (!MatchesCapitalisation(modInput, caseRequirements)) errors.Add($"Capitalisation Does Not Match Format {caseRequirements}!");
                if (charCount > 0 && modInput.Length != charCount) errors.Add($"Input To {(charCount > modInput.Length ? "Short" : "Long")} [{input.Length}], Expected Length [{charCount}]!");
                ...
                ConsoleAction.SendConsoleMessage(new ConsoleLine("--- Input Invalid ---", ConsoleColor.DarkBlue));
