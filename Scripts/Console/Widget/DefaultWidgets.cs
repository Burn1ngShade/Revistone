...
public class TimerWidget : ConsoleWidget // widget for displaying some form of time
{
    ...
    public override string GetContent(ref bool shouldRemove)
    {
        ...
        if (duration <= 0 && !stopwatch)
        {
            ...
            SendDebugMessage(new ConsoleLine($"Timer Has Ended - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(18), AppRegistry.SecondaryCol)));
    ...
    public static void CreateTimer(string name, string time, bool stopwatch = false, int order = 100, bool persistent = true)
    {
        ...
        {
            SendConsoleMessage(new ConsoleLine($"Timer Already Exists - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(23), AppRegistry.SecondaryCol)));
            return;
        }

        if (!NumericalFunctions.TryParseTime(time, out double timerInSeconds))
        {
            SendConsoleMessage(new ConsoleLine("Invalid Time Format Use Format - hh:mm:ss.", BuildArray(AppRegistry.PrimaryCol.SetLength(33), AppRegistry.SecondaryCol)));
            return;
        }

        if (timerInSeconds > 86400)
        {
            SendConsoleMessage(new ConsoleLine("Timer Can Not Exceed Duration - 24 Hours", BuildArray(AppRegistry.PrimaryCol.SetLength(32), AppRegistry.SecondaryCol)));
            return;
        }

        TryAddWidget(new TimerWidget(name, order, (long)(timerInSeconds * 40), true, stopwatch: stopwatch, persistent: persistent));
        SendConsoleMessage(new ConsoleLine($"Timer Created - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(16), AppRegistry.SecondaryCol)));
    }

    /// <summary> Delete timer widget. </summary>
    public static void CancelTimer(string name)
    {
        if (TryRemoveWidget(name)) SendConsoleMessage(new ConsoleLine($"Timer Cancelled - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(18), AppRegistry.SecondaryCol)));
        else SendConsoleMessage(new ConsoleLine($"Timer Does Not Exist - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(23), AppRegistry.SecondaryCol)));
    }
    ...
    public static void TogglePauseTimer(string name)
    {
        ...
        if (consoleWidget is TimerWidget timer)
        {
            ...
            SendConsoleMessage(new ConsoleLine($"Timer {(timer.paused ? "Paused" : "Unpaused")} - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(timer.paused ? 15 : 17), AppRegistry.SecondaryCol)));
        }
        else SendConsoleMessage(new ConsoleLine($"Timer Does Not Exist - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(23), AppRegistry.SecondaryCol)));
    }
    ...
    public static void AdjustTimer(string name, string time)
    {
        ...
        if (consoleWidget is TimerWidget timer)
        {
            ...
            timer.lastUpdate = Manager.ElapsedTicks;
            SendConsoleMessage(new ConsoleLine($"Timer Duration Modified - '{name}'", BuildArray(AppRegistry.PrimaryCol.SetLength(26), AppRegistry.SecondaryCol)));
