        ...
        public class DebitCardApp : App
        {
            ...
            void MainMenu()
            {
                ...
                int userChoice = CreateOptionMenu($"Options",
                         new string[] { "Access Debit Card", "Create Debit Card", "Delete Debit Card",
            ...
            void AccessCard()
            {
                ...
                for (int i = 0; i < cardInfo.Count; i++)
                {
                    if (accountNumber == cardInfo[i].accountNumber.Substring(12))
                    {
                        ...
                        string s = $"Welcome Back To Your [{cardInfo[i].accountType} Account] {cardInfo[i].holderName}!";

                        SendConsoleMessage(new ConsoleLine(s, ColourCreator.HighlightWords(s, new int[] { 4, 5 },
                new ConsoleColor[] { DebitCard.accountColourLookup[(int)cardInfo[i].accountType] }, ConsoleColor.White)));
                        GoNextLine();
                        WaitForUserInput(ConsoleKey.Enter);
            ...
            {
                string card = GetUserInput(new ConsoleLine("Enter A Card Number"), clear: true);

                if (InputValid(card, InputType.Int, 16, -1, true, true))
                {
                    card = card.Replace(" ", "");
            ...
            void LoadDebitCardInfo()
            {
                ...
                {
                    cardInfo.Add(new DebitCard(lines.Skip(i).Take(8).ToArray()));
                }
            }
